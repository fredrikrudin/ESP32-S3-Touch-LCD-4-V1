...
void set_backlight(int percentage) {
    ...
    ledcWrite(PWM_CHANNEL, (percentage * 255) / 100);
}

void update_display_dimming() {
    struct tm ti;
    bool is_night = (getLocalTime(&ti) && (ti.tm_hour >= 22 || ti.tm_hour < 6));
    if (millis() - last_touch_time > 30000) set_backlight(10); else set_backlight(is_night ? 30 : 100);
}

static void global_touch_feedback_cb(lv_event_t * e) {
    last_touch_time = millis();
...
void init_lvgl_interface() {
    ...
    if (inverter.enabled) {
        ...
        lv_obj_set_style_border_color(cap_inv, lv_color_hex(0xE67E22), 0);

        lbl_inv_data_page = lv_label_create(cap_inv);
    ...
    for(int i=0; i<4; i++) {
        btn_relays[i] = lv_btn_create(tab_dash);
        lv_obj_set_size(btn_relays[i], 120, 50);
        ...
        lv_obj_t * lbl = lv_label_create(btn_relays[i]);
        lv_label_set_text_fmt(lbl, "Relä %d", i+1);
        lv_obj_center(lbl);
...
void setup() {
    ...
    server.on("/save_schedule", HTTP_POST, [](AsyncWebServerRequest *request){
        ...
        for(int i=0; i<4; i++) {
            ...
            prefs.putInt(("rsh_" + String(i)).c_str(), relay_sched[i].start_hour);
            prefs.putInt(("rsm_" + String(i)).c_str(), relay_sched[i].start_min);
            prefs.putInt(("reh_" + String(i)).c_str(), relay_sched[i].end_hour);
            prefs.putInt(("rem_" + String(i)).c_str(), relay_sched[i].end_min);
        }
    ...
    server.on("/data", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        struct tm ti; char t_buf[6] = "--:--"; if(getLocalTime(&ti)) sprintf(t_buf, "%02d:%02d", ti.tm_hour, ti.tm_min);
